...
def analyze_column(df, col, target):
    ...
    if df[col].dtype in ['float64', 'int64']:
        # Preencher valores nulos temporariamente
        temp_col = df[col].fillna(0)
        
        # Estatísticas descritivas
        analysis += f"Estatísticas:\n{temp_col.describe()}\n"
        
        # Verificar se a variância não é zero
        if temp_col.std() > 0 and target in df.columns and df[target].std() > 0:
            ...
            reason = "variância zero" if temp_col.std() == 0 else "alvo inválido"
            analysis += f"Correlação com {target}: Não aplicável ({reason})\n"
        ...
        analysis += "Tipo de dado não suportado para análise de correlação.\n"
...
target = "LAID_UP_TIME"
output_path = "feature_analysis_detalhada.txt"
